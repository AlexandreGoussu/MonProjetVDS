...
if (!isset($_SESSION['membre']) && isset($_COOKIE["seSouvenir"])) {
    ...
    $sql = <<<EOD
        Select id, login, nom, titre
        from membre
        where sha2( concat(titre, login, nom, :agent), 256)  = :empreinte
EOD;
    ...
    if($ligne) {
        ...
        $_SESSION['membre']['nomtitre'] = $ligne['titre'] . ' ' . $ligne['nom'];
        // enregistrer la connexion
        Std::enregistrerConnexion($ligne['id']);
        Std::tracerDemande('connexion', $_SESSION['membre']['nomtitre']);


        $option['path'] = '/';
        $option['httponly'] = true;
        $option['expires'] = time() + 3600 * 24 * 7;
        // $empreinte = hash('sha256', $ligne['titre'] .$ligne['login'] . $ligne['nom']);
        $empreinte = hash('sha256', $ligne['titre'] .$ligne['login'] . $ligne['nom'] . $_SERVER['HTTP_USER_AGENT']);
...
<div class="card border-dark mx-2 mb-2">
    ...
    <div class="card-body">
        ...
        <a class="btn btn-sm btn-outline-dark m-2 shadow-sm " href="/agenda/index.php">
            Agenda
        </a>
        ...
        <div id="cadreFFA" style="display :none" class="m-2 border">
            <div class="titre"> Derniers résultats parus sur la page F.F.A</div>
            <div id="detailFFA" class="article">
            </div>
        </div>
