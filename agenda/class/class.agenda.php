...
class Agenda extends Table
{
    public function __construct()
    {
        ...
        // le nom du membre ayant réalisé l'ajout
        // pas de contrôle puisque la donnée n'est pas saisie
        $input = new InputTexte();
        $input->Require = false;
        $input->Unique = false;
        $this->columns['ajouterPar'] = $input;

        // le nom du membre ayant réalisé la modification
        // pas de contrôle puisque la donnée n'est pas saisie
        $input = new InputTexte();
        $input->Require = false;
        $input->Unique = false;
        $this->columns['modifierPar'] = $input;
    ...
    {
        $sql = <<<EOD
			Select id, nom, date_format(date, '%d/%m/%Y') as dateFr, if(date < current_date, 1, 0) as old, ajouterPar, modifierPar, type
            From agenda
            Order by date desc;
EOD;
